"""HospitalityNet RSS feed scraper."""
...
class HospitalityNetScraper(BaseScraper):
    """Scraper for HospitalityNet RSS feed."""

    source_name = "hospitalitynet"
    source_type = SourceType.NEWS

    RSS_URL = "https://www.hospitalitynet.org/news.rss"
    ...
    def scrape(self) -> list[RawContentCreate]:
        """Scrape articles from HospitalityNet RSS feed.
        ...
        logger.info(f"Scraped {len(items)} articles from HospitalityNet")
        return items


def scrape_hospitalitynet() -> dict:
    """Convenience function to run the scraper.

    Returns:
        Scrape summary dict
    """
    with HospitalityNetScraper() as scraper:
        return scraper.run()


if __name__ == "__main__":
    logging.basicConfig(level=logging.INFO)
    result = scrape_hospitalitynet()
    print(result)
